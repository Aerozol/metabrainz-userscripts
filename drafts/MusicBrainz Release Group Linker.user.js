...
// @description  Copy RYM or Discogs artist data to clipboard and compare in MB artist pages. Click to add links REQUIRES the MusicBrainz: 'add release(group) links from artist/label page' userscript.
// @license      MIT
// @version      1.04
// @author       Gemini
...
(function() {
    ...
    style.innerHTML = `
        .rym-stealth-hide { display: none !important; }
    ...
    if (window.location.host.includes('discogs.com')) {
        ...
        harvestBtn.on('click', function() {
            ...
            $('tr[class*="textWithCoversRow"]').each(function() {
                const row = $(this);
                // Specifically find the anchor that contains "/master/"
                ...
                // Traverses up to find the closest table, then the preceding header
